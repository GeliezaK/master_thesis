...
def merge_small_and_large_roi_df(df_small, df_large, outpath):
    """Merge two dataframes by date and save to outpath. """
    # Extract date (assuming system:start_time is present and ISO format)
    df_small["date"] = pd.to_datetime(df_small["system:time_start"], unit="ms").dt.date
    df_large["date"] = pd.to_datetime(df_large["system:time_start"], unit="ms").dt.date

    # Keep only the needed columns
    df_small = df_small[["date", "cloud_cover"]].rename(columns={"cloud_cover": "cloud_cover_small"})
    df_large = df_large[["date", "cloud_cover"]].rename(columns={"cloud_cover": "cloud_cover_large"})

    # Merge on date
    df_merged = pd.merge(df_small, df_large, on="date", how="inner")

    # Save to CSV
    df_merged.to_csv(outpath, index=False)

    print(f"Merged dataframe saved to {outpath}")
    print(df_merged.head())
    

def merge_all_cloud_cover_tables(input_folder="data/S2_cloud_cover_tables", 
                                 output_file="data/s2_cloud_cover_table_small_and_large.csv"):
...
if __name__ == "__main__":
    ...
    merged_outpath = "data/s2_cloud_cover_table_small_and_large.csv"
